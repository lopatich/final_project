...
def menu():
    ...
    canv.create_text(canv.winfo_width()/2, canv.winfo_height()/2 + 30,text="Press ENTER to start.", fill="white", font=otherFont)
def draw():
    canv.delete("all")
    if gameState:
        canv.delete("all")
        ...
        if dead:
            ...
            canv.create_text(canv.winfo_width()/2, canv.winfo_height()/2 + 30,text="ROUNDS SURVIVED: " + str(wavesSurvived),fill="green", font=gOver)
    else:
        drawBackground()
        menu()
    root.after(25, start)
